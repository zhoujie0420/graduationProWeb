...
export async  function initializePeerPlugin() {
    return new Promise(resolve => {
        let peerStore = usePeerStore();
        let phone = peerStore.phone;
        let { host, port, path } = { ...networkConfiguration.server.peerServer };
        let localPeer = new Peer(phone,{ host, port, path });
        ...
        localPeer.on("error", error => {
            console.error(
                "localPeer error,the error information is : ",
